...
- Added interactive builtin shell (command `loza @shell`) (GH-113)
...
- Added `lozaexe` variable to module `sys` (GH-92)
- Added a small internal web server for loza in module `web.server` (GH-92)
- Added base of web development features for backend with loza (module `@web`) (GH-92)
- Added a cli behavior for `time` module to show current time (GH-89)
- Added `null` keyword as a alias for `None` (GH-90)
- Moved `pit(lozahtml)` engine from a external library to a builtin module (GH-85)
- Added shutdown event system and function `register_shutdown()` (GH-91)
- Added `re` python regex library
- Added function `die`
- Moved `lozailer` from a external library to a builtin module and renamed to `compiler` (GH-93)
