...
def load(path: str, code_location: str, self=None, is_jit_disabled=False, ismain_default=False) -> list:
    ...
    try:
        is_jit_disabled = bool(os.environ['LOZA_DISABLE_JIT'])
    except:
        try:
            if os.path.isdir(os.path.dirname(os.path.abspath(path)) + '/__lozaam__'):
                if not os.access(os.path.dirname(os.path.abspath(path)) + '/__lozaam__', os.W_OK):
    ...
    try:
        ...
        the_cache_dir = os.path.dirname(os.path.abspath(path)) + '/__lozaam__'
