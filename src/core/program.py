...
class Program(helpers.Helpers):
    ...
    def __init__(self, is_test=False, args=[]):
        ...
        self.allowed_loza_extensions = ['loza']
                    ...
                    if not is_currently_imported:
                        try:
                            ...
                            for path in self.module_path:
                                ...
                                if os.path.isfile(full_path + '.loza'):
                                    commands = jit.load(os.path.abspath(full_path + '.loza'), os.path.abspath(full_path + '.loza'), self, ismain_default=ismain_default)
                                elif os.path.isdir(full_path):
                                    if os.path.isfile(os.path.abspath(full_path + '/__init__.loza')):
                                        commands = jit.load(os.path.abspath(full_path + '/__init__.loza'), os.path.abspath(full_path + '/__init__.loza'), self, ismain_default=ismain_default)
                            if commands == False:
                                raise KeyError()
    ...
    def bootstrap_modules(self):
        ...
        os.environ['LOZAPATH'] = '/usr/lib/loza_modules;' + home_directory + '/.local/lib/loza_modules;' + os.environ['LOZAPATH']
        loza_module_paths = os.environ['LOZAPATH']
        paths = loza_module_paths.strip().split(';')
